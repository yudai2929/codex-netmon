...
func (generator Generator) Run() error {
	...
	manifest, err := BuildWiFiDashboard()
	if err != nil {
		return fmt.Errorf("build Wi-Fi dashboard: %w", err)
	}
	data, err := json.MarshalIndent(manifest, "", "  ")
	if err != nil {
		return fmt.Errorf("marshal Wi-Fi dashboard: %w", err)
	}
	...
	if err := os.WriteFile(generator.OutputPath, append(data, '\n'), 0o644); err != nil {
		return fmt.Errorf("write Wi-Fi dashboard: %w", err)
	}
	return nil
}
